    ...
    try:
        if not nownow()//15 == nowmin :
            ...
            if LP == True:
                sellbit(etheramount)
                LP = False
            if not nowH == float(nowhour())//2:
                profit = BGDF()/SM
                TEXT = 'it is...'
                PN = 'profit till now...' + str(profit)
                mail(TEXT,PN)
                nowH = float(nowhour())//2
            btcc()
            GetPD()
            nowmin = nownow()//15
            EtherPrice = GetPD()

            IV = EtherPrice[['high1', 'low1', 'close1','open1', 'open2', 'high2', 'low2', 'close2','tMA1','RR','bollow1','bolhigh1']]
            DV = EtherPrice[['close']]
            ...
            pos15 = False
        # 숏잡을때
        if float(ls3[-1]) > float(ls1[-1]) and SP == False and pos15 == False and getcurrent() > ls3[-1] *1.0005 :
            ...
            pos15 = True
        #숏 정리 
        if SP == True and getcurrent() < sellprice * 0.9985 :
            buybit(etheramount)
            SP = False
            print(datetime.now(),'short possition END')
        # 롱 잡을 때 
        if float(ls3[-1]) < float(ls1[-1]) and LP == False and pos15 == False and getcurrent() < ls3[-1] * 0.9995 :
            buyprice = getcurrent()
            etheramount = amountgetter()
            buybit(etheramount)
            ...
            pos15 = True
        # 롱 정리 
        if LP == True and getcurrent() > buyprice * 1.0015:
            sellbit(etheramount)
            LP = False
