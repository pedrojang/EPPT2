...
while True:
    try:
        if not nownow()//15 == nowmin:
            ...
            pos15 = False
        # 숏잡을때
        if ls3[-1] > ls1[-1] and SP == False and pos15 == False:
        ...
        if SP == True and getcurrent() < sellprice * 0.999 :
            buybit(etheramount)
            SP = False
            print(time.now(),'short possition END')
        # 롱 잡을 때 
        if ls3[-1] < ls1[-1] and LP == False and pos15 == False:
            buyprice = getcurrent()
            etheramount = amountgetter()
            buybit(etheramount)
            print(time.now(),'buy')
        ...
        if LP == True and getcurrent() > buyprice * 1.001:
            sellbit(etheramount)
            LP = False
            print(time.now(),'long possition END')
